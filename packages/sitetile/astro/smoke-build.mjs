...
// by slug). Proves the gap confirmed live on atelier-a.co.jp 2026-09-02: the base-locale-only
...
const checks = [
  ...
  // -- atelier-a /faq, 2026-09-03: the bubble's panel header is the STORE's stable name, and must
  // stay that on every page — not each page's own composed <title> (was measured live as
  // "常見問題 | atelier-a【新官網轉移中】", the FAQ page's own title text, on a page whose title has
  ...
  // -- nav-mobile open menu vs. an unlayered theme z-index (atelier-a.co.jp, live): the dropdown
  ...
  ['nav-mobile: the open header is lifted above an unlayered theme z-index with !important, not just a specific selector (atelier-a.co.jp regression)', () => {
    const css = allCss().replace(/\s+/g, '');
  ...
  ['nav-mobile (hamburger): the open panel scrolls itself when taller than the viewport (952px panel vs 720px viewport, atelier-a.co.jp — body is scroll-locked above, so without this the tail of the menu is unreachable), and the position/z-index FACTS that keep it a floating panel stay !important', () => {
    const css = allCss().replace(/\s+/g, '');
  ...
  // atelier-a.co.jp/zh-tw/contact/. The markup-level check above can't catch this — `hidden` was
  ...
  // -- Lingo × Blog locale category/tag archives (the atelier-a gap, 2026-09-02) --
  ...
  // on feelreef/site-atelier-a, 585/585 both zh-tw and en-us).
