...
// own (atelier-a /faq, 2026-09-03: og:site_name and the inbox bubble's data-site-name both rendered
// "よくある質問 | atelier-a【新HP移行中】", the PAGE's own composed <title>, because `site-title`
...
check('og:site_name prefers `brand`/`footer-brand` over headerBrand\'s page-title fallback', () => {
  ...
  const pageComposedTitle = 'よくある質問 | atelier-a【新HP移行中】';
  assert.equal(resolve({ brand: 'atelier-a' }, pageComposedTitle), 'atelier-a');
  assert.equal(resolve({ 'footer-brand': 'atelier-a' }, pageComposedTitle), 'atelier-a');
...
mustFail('the og:site_name brand/footer-brand precedence check', () => {
  ...
  // eslint-disable-next-line no-new-func
  const resolve = (meta, headerBrand) => new Function('meta', 'headerBrand', body)(meta, headerBrand);
  assert.equal(resolve({ brand: 'atelier-a' }, 'よくある質問 | atelier-a【新HP移行中】'), 'atelier-a');
});
