...
- **raw HTML** — every tag is escaped to visible text (`<script>` → `&lt;script&gt;`), except two allowlisted, attribute-free tags re-emitted live (`<small>`, `<br>`), and **HTML comments** (`` <!-- … --> ``, or the HTML spec's alternative closer `--!>` and its abrupt forms `<!-->` / `<!--->` / `<!---->`), which are removed rather than shown — a QA/author note in page markdown must never leak onto the public page (issue #496). To show a literal `<!-- … -->`, put it in a code span or fenced block: those are code, not comments (a `\` before it is **not** an escape hatch — `\<!-- x -->` still has its comment removed). An **unterminated** `<!--` (no closer anywhere) deletes the rest of the document, per the HTML tokenizer's own comment-end state — close every comment you open.
