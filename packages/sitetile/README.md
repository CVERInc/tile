...
- **raw HTML** — every tag is escaped to visible text (`<script>` → `&lt;script&gt;`), except two allowlisted, attribute-free tags re-emitted live (`<small>`, `<br>`), and **HTML comments** (`` <!-- … --> ``, or the HTML spec's alternative closer `--!>` and its abrupt forms `<!-->` / `<!--->` / `<!---->`), which are removed rather than shown — a QA/author note in page markdown must never leak onto the public page (issue #496). A comment recognized at the START of a line (after any list/quote marker) is a BLOCK, exactly like a fence or a heading: it can span blank lines, list items, anything, until a line containing its closer; a comment elsewhere is removed inline, within its own paragraph/heading/list-item text. To show a literal `<!-- … -->`, put it in a code span or fenced block: those are code, not comments (a `\` before it is **not** an escape hatch — `\<!-- x -->` still has its comment removed). An **unterminated** `<!--` (no closer anywhere in the document) is left as literal, visible, escaped text — nothing disappears silently; close the comment to make it disappear.
